...
peakpickingParameters <- getDefaultXcmsSetStartingParams('matchedFilter') #only matchedFilter
...
time.xcmsSet <- system.time({ 
  resultPeakpicking <- 
    optimizeXcmsSet(files = datafiles[1:6], # check datafiles object. Input only the files to be tested
                    params = peakpickingParameters, 
                    nSlaves = 1,# only change if your computer can handle it. (Processing )
...
# Save the R image to keep the objects stored
...
# the script generated by IPO is similar to the one below. Substitute this chunk with the one generated by IPO and rename the objects
...
xset2 <- retcor( 
  ...
  plottype       = "none",
  distFunc       = "cor_opt",
  profStep       = 1, #possible bug, if so, change it to .99
  center         = 4,
  response       = 1,
  gapInit        = 0.528,
  gapExtend      = 2.7,
